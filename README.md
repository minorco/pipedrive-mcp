...
`pipedrive_files_list` scopes through the per-entity files endpoints (exactly one of `deal_id`, `person_id`, `org_id`, `product_id`); `activity_id`, `lead_id` and `mail_message_id` filter within that scope. Files expose `mail_message_id`; passing `mail_message_id` alone lists the attachments of one email (Pipedrive keeps email attachments unlinked to deals, so the tool locates them by the message's sync time; inline images hidden unless `include_inline`).
